      ...
      <div className="mx-auto max-w-7xl px-6">
        ...
        <div className="grid gap-8 lg:grid-cols-3">
          ...
          <div className="lg:col-span-2 space-y-4">
            {items.map(item => (
              <div key={item.id} className="flex gap-4 rounded-2xl bg-white p-4 shadow-lg">
                <img 
                  src={`https://picsum.photos/id/${item.imageId || 100}/100/100`} 
                  alt={item.name}
                  ...
                  <div className="flex items-start justify-between">
                    <div>
                      <Link to={`/product/${item.id}`}>
                        <h3 className="font-semibold text-gray-900 hover:text-primary-600">{item.name}</h3>
                      </Link>
