...
const categories = [
  ...
  { name: 'Furniture', icon: '🪑', color: 'bg-stone-100' },
];

const CategoryBar = ({ selectedCategory, onSelectCategory, showCount = false }) => {
      ...
      <div className="flex gap-3 overflow-x-auto pb-2 scrollbar-hide">
        ...
        {categories.map(cat => (
          ...
          >
            ...
            {showCount && categoryCounts[cat.name] && (
              <span className="text-[10px] opacity-70">{categoryCounts[cat.name]}</span>
            )}
          </button>
        ))}
      </div>
